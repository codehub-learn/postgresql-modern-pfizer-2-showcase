...
$$
SELECT *
FROM pgmq.set_vt(queue_name, msg_id, clock_timestamp() + make_interval(secs => vt));
$$ LANGUAGE sql;
...
$$
SELECT *
FROM pgmq.set_vt(queue_name, msg_ids, clock_timestamp() + make_interval(secs => vt));
$$ LANGUAGE sql;
...
DECLARE
    queue_record RECORD;
BEGIN
    FOR queue_record IN SELECT queue_name FROM pgmq.meta
        LOOP
...
BEGIN
    ...
    IF a_partition_col = 'archived_at' THEN
        EXECUTE format(
                'CREATE TABLE pgmq.%I (LIKE pgmq.%I including defaults including generated including storage including comments) PARTITION BY RANGE (%I)',
                a_table_name, a_table_name_old, a_partition_col);
    ELSE
        EXECUTE format('CREATE TABLE pgmq.%I (LIKE pgmq.%I including all) PARTITION BY RANGE (%I)', a_table_name, a_table_name_old,
                       a_partition_col);
    END IF;
...
$$
SELECT *
FROM pgmq.send_batch_topic(routing_key, msgs, NULL, clock_timestamp());
$$;
...
$$
SELECT *
FROM pgmq.send_batch_topic(routing_key, msgs, headers, clock_timestamp());
$$;
...
$$
SELECT *
FROM pgmq.send_batch_topic(routing_key, msgs, NULL, clock_timestamp() + make_interval(secs => delay));
$$;
...
$$
SELECT *
FROM pgmq.send_batch_topic(routing_key, msgs, NULL, delay);
$$;
